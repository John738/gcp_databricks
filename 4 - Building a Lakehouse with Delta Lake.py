...
# MAGIC %sql
# MAGIC CREATE DATABASE IF NOT EXISTS odl_user_564377
...
# MAGIC %fs
# MAGIC mkdirs /tmp/odl_user_564377/delta/clickstream
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS odl_user_564377.clickstream_data;
# MAGIC CREATE TABLE odl_user_564377.clickstream_data (
...
# MAGIC USING DELTA
# MAGIC LOCATION '/tmp/odl_user_564377/delta/clickstream';
# MAGIC 
# MAGIC ALTER TABLE odl_user_564377.clickstream_data ADD CONSTRAINT dateWithinRange CHECK (event_time > '2015-01-01');
# MAGIC ALTER TABLE odl_user_564377.clickstream_data ADD CONSTRAINT validCountryCode CHECK (country in ('MX','TW','PE','CO','UY','PR','CL','ID','CA','GB','JP','US','PA'));
# MAGIC ALTER TABLE odl_user_564377.clickstream_data DROP CONSTRAINT dateWithinRange;
# MAGIC ALTER TABLE odl_user_564377.clickstream_data DROP CONSTRAINT validCountryCode;
...
output = (
           parsed_fields.write.format("delta")  # Specify the format as Delta
             ...
             .saveAsTable("odl_user_564377.clickstream_data") ## Specify table name or specify a file path if no table is created and you want to write to any directory
         )
...
# MAGIC %sql
# MAGIC ANALYZE TABLE odl_user_564377.clickstream_data COMPUTE STATISTICS FOR COLUMNS domain_name, event_time, user_id, country, division, page_url, page_app, page_id, browser, os, platform
...
# MAGIC %sql
# MAGIC DESCRIBE FORMATTED odl_user_564377.clickstream_data;
...
# MAGIC %fs
# MAGIC ls /tmp/odl_user_564377/delta/clickstream
...
# MAGIC %sql
# MAGIC OPTIMIZE odl_user_564377.clickstream_data;
...
# MAGIC %sql
# MAGIC select * from odl_user_564377.clickstream_data
...
# MAGIC %sql
# MAGIC UPDATE odl_user_564377.clickstream_data SET event_time = '2022-01-01 14:11:32', domain_name= "TEST UPDATE DOMAIN NAME";
...
# MAGIC %sql
# MAGIC SELECT COUNT(1) FROM odl_user_564377.clickstream_data;
...
# MAGIC %sql
# MAGIC MERGE INTO odl_user_564377.clickstream_data a /** This is the target table **/
...
# MAGIC %sql
# MAGIC DELETE FROM odl_user_564377.clickstream_data;
# MAGIC SELECT * FROM odl_user_564377.clickstream_data 
...
# MAGIC %sql
# MAGIC DESCRIBE HISTORY odl_user_564377.clickstream_data;
...
# MAGIC %sql
# MAGIC INSERT INTO odl_user_564377.clickstream_data
# MAGIC SELECT * FROM odl_user_564377.clickstream_data VERSION AS OF 5 /** Can also specify the timestamp instead of the version **/
...
# MAGIC VACUUM odl_user_564377.clickstream_data RETAIN 0 HOURS;
